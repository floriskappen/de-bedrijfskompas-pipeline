"""CLI entry point: ``python -m pipeline.global_scoring``."""
...
DEFAULT_OUT_DIR = Path("data/global-scoring")


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m pipeline.global_scoring",
        description="Score each company on the five framework axes from its dossier.",
    )
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        ...
        help=f"Where to write per-company score records (default: {DEFAULT_OUT_DIR}).",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    for result in run(
        ...
        print(
            ...
            f"{_score_summary(result.get('scores'))}",
            file=sys.stderr,
        )
...
def _score_summary(scores: dict | None) -> str:
    """One-line per-axis score digest for the stderr progress log."""
    if not isinstance(scores, dict):
        return ""
    return " ".join(f"{a[:3]}={(v or {}).get('score')}" for a, v in scores.items())
