"""CLI entry point: ``python -m pipeline.content_summarization``."""
...
DEFAULT_OUT_DIR = Path("data/content-summarization")


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m pipeline.content_summarization",
        description="Generate a faithful, de-marketed English dossier per company.",
    )
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        ...
        help=f"Where to write per-company dossiers (default: {DEFAULT_OUT_DIR}).",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    for result in run(
        ...
        print(
            ...
            f"lang={result.get('source_language')} chars={body_len}",
            file=sys.stderr,
        )
