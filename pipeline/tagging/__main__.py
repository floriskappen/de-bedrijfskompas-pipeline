"""CLI entry point: ``python -m pipeline.tagging``."""
...
DEFAULT_OUT_DIR = Path("data/tagging")


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m pipeline.tagging",
        description="Tag each company with capability families drawn from the dossier.",
    )
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        ...
        help=f"Where to write per-company tagging records (default: {DEFAULT_OUT_DIR}).",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    for result in run(
        ...
        tags = result.get("capability_tags") or []
        print(
            f"{result.get('name', '?')}: status={status} tags={len(tags)}",
            file=sys.stderr,
        )
