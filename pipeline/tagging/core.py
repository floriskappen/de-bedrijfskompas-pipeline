"""Per-company capability tagger and batch runner for the tagging stage."""
...
PROMPT_PATH = Path(__file__).resolve().parents[2] / "prompts" / "tagging.md"
...
def process(
    ...
    *,
    ...
    """Tag one company. Raises only on company-id collision."""
    if meta.get("status") != "ok":
        ...
        try:
            tags = llm_module.call(build_messages(body))
            result = _record(
                meta,
                status="ok",
...
def _resolve_concurrency() -> int:
    """Bound the per-stage LLM pool: ``TAGGING_CONCURRENCY`` env, default 8."""
    raw = os.environ.get("TAGGING_CONCURRENCY")
    if raw is None:
...
def run(
    ...
    *,
    ...
    """Yield one tagging record per company, in input order. Never raises on per-company load errors.

    Companies are processed concurrently in a bounded pool (``TAGGING_CONCURRENCY``);
...
def load_prompt() -> str:
    """Load the tagging prompt from the versioned file under ``prompts/``."""
...
def _record(
    ...
    *,
    ...
    return {
        ...
        "status": status,
        "model": model,
        "capability_tags": capability_tags if status == "ok" else None,
    }
