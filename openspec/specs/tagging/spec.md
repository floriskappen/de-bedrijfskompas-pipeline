...
## Purpose
Specify the dossier-derived analytic stage that tags each company with a fixed-vocabulary set of capability families (`{ family, prominence }`), drawn from the content-summarization dossier, for use by the frontend as a faceted-filter signal.
