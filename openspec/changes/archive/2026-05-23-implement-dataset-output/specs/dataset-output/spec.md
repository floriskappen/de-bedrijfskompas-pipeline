...
- **Dutch lookup is by flat dotted key.** The translation stage stores `translations` keyed by the dotted target path (`"scores.substance.reason"`), not as a nested tree. Resolve `nl` values by that exact key; a nested traversal will silently find nothing.
- **Use the shared `company_id` helper.** The filename stem and the `company_id` field must come from the same slug helper the upstream stages use, or the spine join will miss files whose slug differs.
